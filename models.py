            ...
            for t in range(decode_step):
                for prev_output in prev_top_outputs:
                    ...
                    if self.joint_ctc:
                        ...
                        ctc_prob, ctc_state = ctc_prefix.cheap_compute(prev_output.outIndex, prev_output.ctc_state,
                                                                       candidates)
...
class Speller(nn.Module):
    def __init__(self, input_dim, dim, layer, rnn_cell, dropout):
        ...
        self.layer0 = nn.GRU(input_dim, dim)
        for i in range(1, layer):
            setattr(self, 'layer' + str(i), nn.GRU(dim, dim))
    ...
    def forward(self, input_context):
        self.state_list[0], self.cell_list[0] = self.layer0(self.dropout(input_context),
                                                            (self.state_list[0], self.cell_list[0]))
        for l in range(1, self.layer):
            self.state_list[l], self.cell_list[l] = getattr(self, 'layer' + str(l))(self.state_list[l - 1], (
                self.dropout(self.state_list[l]), self.cell_list[l]))
...
class Attention(nn.Module):
    ...
    def forward(self, decoder_state, listener_feature, state_len, scale=2.0):
        ...
        if self.mode == 'dot':
            ...
            energy = self.gen_energy(
                torch.tanh(self.comp_listener_feature + comp_decoder_state + comp_location_info)).squeeze(2)
            energy.masked_fill_(self.state_mask, -float("inf"))
            attention_score = [self.softmax(energy * scale)]
...
class RNNLayer(nn.Module):
    def __init__(self, in_dim, out_dim, sample_rate, sample_style='drop', layers=1,
        ...
        self.gru = nn.GRU(in_dim, out_dim, bidirectional=bidir, num_layers=layers, dropout=dropout_rate, batch_first=True)
    ...
    def forward(self, input_x, state=None, state_len=None, pack_input=False):
        ...
        output, hidden = self.gru(input_x, state)
        if pack_input:
